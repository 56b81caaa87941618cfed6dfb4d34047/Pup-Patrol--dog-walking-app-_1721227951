/* Summary: This appears to be a JavaScript file that sets up various interactive and animation-related functionalities for a web page.
1. **AOS.init()**: This sets up the Animate on Scroll (AOS) library with specific configuration options, such as triggering the animation only once, disabling the animation on phones, and setting the duration and easing of the animation.
2. **Clients Carousel**: This sets up a Swiper.js carousel for the clients section, with automatic sliding, centered slides, and no swiping.
3. **Stellar Carousel**: This sets up another Swiper.js carousel for the "stellar" section, with different slide counts based on screen size, and navigation buttons.
4. **Particle Animation**: This creates a custom particle animation system using a `ParticleAnimation` class. It initializes the canvas, creates and updates the particle positions based on mouse movement, and handles the animation loop.
5. **Box Highlighter**: This creates a "box highlighter" effect, where the boxes in a container are highlighted based on the user's mouse position relative to the container.
Overall, this file sets up various interactive and visual effects for a web page, including carousels, particle animations, and box highlights. */
